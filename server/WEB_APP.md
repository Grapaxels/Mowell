...
2.4.4 loads the same authenticated WebRTC ICE configuration used by the web
client (`versionCode` 43) and keeps API responses separate from the web SPA.
...
Mowell first attempts peer-to-peer WebRTC and uses the Grapaxels TURN relay on
port 3478 over UDP or TCP when a direct route is unavailable. Authenticated web
...
present in repository source and must also be delivered to WebRTC clients,
protect the TURN server with bandwidth limits and rotate the credential in the
TURN configuration and `callIceServers` if it is exposed.
...
The signed 2.4.4 APK is included in Vercel's `server/public` directory and is
served as a static asset at
`https://mowell-api.grapaxels.in/Mowell-v2.4.4.apk`, avoiding private GitHub
